...
class CSVStructurePopulation:
    def __init__(self, filename, addDate=False, info=False, offset=0):
        self.df = self.CSV2DataFrame(filename, addDate, info, offset=offset)

    @staticmethod
    def CSV2DataFrame(filename, addDate=False, info=False, offset=0):
        uploadTime = None
        if (addDate):
        ...
        headers = CSVStructurePopulation.none_replace(ws.head(3))

        df = pd.read_excel(filename, skiprows=[0, 1, 2], header=None, engine='openpyxl')
